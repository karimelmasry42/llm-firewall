...
Exposes both a scanning filter (`PiiFilter` returning a `FilterResult`) and a
standalone `mask` function that replaces detected entities in place. Both share
the same pattern set so the detection and masking surfaces stay consistent.
"""
...
def mask(text: str, mask_char: str = "*") -> PIIMaskResult:
    """Mask detected PII in text using the shared regex pattern set."""
    masked_text = text
...
class PiiFilter:
    """Scans text for PII and returns a binary FilterResult."""

    FILTER_NAME = "PII"

    def scan(self, text: str) -> FilterResult:
        """Return passed=False when any PII pattern matches."""
        detected: list[str] = []
        for entity_type, pattern in _COMPILED_PATTERNS.items():
