...
Every dot above is one real DavidTKeane prompt scored by the shipped, threshold-tuned classifier. Benign English (green) clusters two orders of magnitude *below* the threshold; English injections (red) mostly sit above it but with a visible false-negative tail — that's the 0.807 English F1 in [`prompt_guard_2.json`](data/input_classifier/eval/prompt_guard_2.json#L4) made visible. Non-English injections in French, German, Spanish, Chinese, and Japanese sit above the line on essentially the same model output range. Sample sizes outside English are tiny (annotated as `n=…`) so don't read this as proof — the multilingual F1 of **0.824** on the full 112-prompt benchmark in the bake-off above is the real evidence; this chart just shows what the per-prompt scores behind that number look like.

For a closer look at individual non-English prompts and the actual scores returned (including a deliberate false-negative French example), see:
...
The figures are regenerated by [`scripts/input_classifier/generate_visualizations.py`](scripts/input_classifier/generate_visualizations.py) from the per-prompt parquet that [`scripts/input_classifier/evaluate.py`](scripts/input_classifier/evaluate.py) writes alongside its JSON report — no hard-coded constants in the chart code.
